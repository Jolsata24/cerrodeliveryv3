...
// Restauramos 'r.telefono' a la consulta para el botón de WhatsApp
...
if ($resultado_pedidos->num_rows > 0):
    while ($pedido = $resultado_pedidos->fetch_assoc()):
        ...
        // Textos restaurados a la idea original
        switch ($pedido['estado_pedido']) {
            case 'Pendiente': 
                $progreso = 15;
                $estado_actual = 'Esperando confirmación...'; 
        ...
        // RESTAURAMOS EL BOTÓN DE WHATSAPP
        $btn_whatsapp = '';
    ...
    <div class="card order-card mb-4 shadow-sm">
        ...
        <div class="card-body">
            <div class="mb-3">
                ...
                <div class="progress" style="height: 12px; border-radius: 10px;">
                    <div class="progress-bar progress-bar-striped progress-bar-animated bg-<?php echo $estado_clase; ?>" 
                         role="progressbar" 
                         style="width: <?php echo $progreso; ?>%" 
                         aria-valuenow="<?php echo $progreso; ?>" aria-valuemin="0" aria-valuemax="100">
                    </div>
                </div>
                <small class="text-muted mt-2 d-block"><i class="bi bi-info-circle me-1"></i><?php echo $estado_actual; ?></small>
            </div>
        </div>
        ...
        <?php 
        $estados_rastreables = ['En preparación', 'Listo para recoger', 'En camino'];
        if (in_array($pedido['estado_pedido'], $estados_rastreables)): 
        ?>
        <div class="card-footer bg-white text-center border-0 pt-0 pb-3">
            <a href="rastrear_pedido.php?id_pedido=<?php echo $pedido['id']; ?>" class="btn btn-outline-primary btn-track-order w-100 fw-bold">
                <i class="bi bi-geo-alt-fill me-2"></i> Rastrear Motorizado en el Mapa
            </a>
        </div>
        <?php endif; ?>
    </div>
